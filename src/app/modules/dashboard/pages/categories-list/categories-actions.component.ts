    ...
    template: `
      <ng-container>
        <div *ngIf="cellValue?.data?.createdBy === currentUser?.id || userRoleCheckAdmin == true" class="flex justify-start gap-x-2 mt-2">
          <a (click)="showAddOrEditDialog(addTemplate, cellValue?.data)"  class="w-10 h-6 px-1.5 py-1 text-xs font-medium text-center text-blue-500 no-underline bg-white rounded-md cursor-pointer hover:text-blue-600">
