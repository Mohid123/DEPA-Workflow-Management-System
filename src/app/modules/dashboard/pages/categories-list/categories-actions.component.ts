    ...
    template: `
      <ng-container>
        <div *ngIf="cellValue?.data?.createdBy === currentUser?.id || userRoleCheckAdmin == true" class="flex justify-start gap-x-2 mt-2">
          <a (click)="showAddOrEditDialog(addTemplate, cellValue?.data)"  class="w-7 h-6 px-1 py-1 text-xs font-medium text-center text-white no-underline bg-blue-500 rounded-md cursor-pointer hover:text-white">
