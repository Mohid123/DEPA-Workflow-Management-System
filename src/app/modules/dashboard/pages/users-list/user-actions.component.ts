...
@Component({
    ...
    template: `
      ...
      <div [ngClass]="checkRoles(cellValue?.data?.roles) == false ? 'block': 'hidden'" class="flex justify-start gap-x-3 mt-2">
        <a (click)="showDialog(template, cellValue?.data)" class="w-7 h-6 px-1 py-1 text-xs font-medium text-center text-white no-underline bg-blue-500 rounded-md cursor-pointer hover:text-white">
